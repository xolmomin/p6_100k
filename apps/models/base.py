from django.db.models import BooleanField, Model, DateTimeField, SlugField, ForeignKey, PROTECT, TextField, CharField, \
    CASCADE, IntegerField, Manager
from django_resized import ResizedImageField


class Product(Model):
    title = CharField(max_length=255)
    main_picture = ResizedImageField(size=[500, 300], upload_to='product/%m')
    price = IntegerField()
...
class Stream(Model):
    name = CharField(max_length=255)
    donation = CharField(max_length=20, default=0)  # hayriya uchun mablag`
    reduce = CharField(max_length=20, default=0)  # narxini kamaytirish uchun mablag`
